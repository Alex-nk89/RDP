...
import { useExtractedMargins, useCss, useMantineTheme } from '@mantine/styles';
...
function useBoxSx(sx, className) {
  const { css, cx } = useCss();
  const theme = useMantineTheme();
  if (Array.isArray(sx)) {
    return cx(className, sx.map((partial) => css(extractSx(partial, theme))));
  }
...
const Box = forwardRef((_a, ref) => {
  ...
  return /* @__PURE__ */ React.createElement(Element, __spreadValues({
    ref,
    className: useBoxSx(sx, className),
    style: mergedStyles
  }, rest));
});
